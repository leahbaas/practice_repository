...
for x in ACMG59_diseases:
    b = x.strip('"').replace('-',' ').translate(str.maketrans('', '', string.punctuation)).strip().lower().split()   # 2/7/2018: This line was giving an error when run w/ Python3 b/c the translate() method works differently in Python3 vs Python2. Modified to resolve error.
    for y in b:
        if y in stopwords:
            b.remove(y)
...
with open('matches_updated.txt', 'w') as file:
