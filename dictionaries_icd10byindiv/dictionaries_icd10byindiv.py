...
#     icd10 = sheet.cell_value(row, 1)
#     dscrptn = sheet.cell_value(row, 2)
#     dx_date = sheet.cell_value(row, 3)
#     data = []
#     if isinstance (dx_date, float):
#         def date_correct(dx_date):
#             d = xlrd.xldate_as_tuple(dx_date, workbook.datemode)
#             d = datetime.datetime(*(d[0:3]))
#             return d.strftime('%m-%d-%Y')
#         dStr = date_correct(dx_date)
#         data.extend([icd10, dscrptn, dStr])
#     else:
...
# pickle_out = open("/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_mrn.pickle", "wb")
...
#     if isinstance (dx_date, float):
#         dStr = date_correct(dx_date)
#         data.extend([sheet.cell_value(row,0), sheet.cell_value(row,2), dStr])
#     else:
#         data.extend([sheet.cell_value(row,0), sheet.cell_value(row,2), sheet.cell_value(row,3)])
#     #print(data)
#     for code in codes:
...
# pickle_out = open("/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_code.pickle", "wb")
...
pickle_in_mrn = open("/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_mrn.pickle", "rb")
test_mrn = pickle.load(pickle_in_mrn)
#print(test_mrn)

pickle_in_code = open("/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_code.pickle", "rb")
test_code = pickle.load(pickle_in_code)
#print(test_code)

#Step 4: Write dictionaries to text files so that you can view the output
with open('/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_mrn.txt', 'w') as file:
    file.write(json.dumps(test_mrn, indent=5))

with open('/home/leah/src/practice_repository/dictionaries_icd10byindiv/dict_code.txt', 'w') as file:
    file.write(json.dumps(test_code, indent=5))
#
# #Step 5: Sanity Checks
#
# #   1. Check to make sure that xlrd read the complete xcel file by counting number of unique mrns (should be 338):
#
# mrns = []
# for row in range(1, sheet.nrows):
#     mrn = sheet.cell_value(row, 0)
...
#
# #   3. Check to make sure number of unique icd-10 codes in dict_code = number of unique_codes in spreadsheet:
#         # a. Determine number of unique icd-10 codes in xcel file
# code_list = []
# for row in range(1, sheet.nrows):
#     codes = str(sheet.cell_value(row, 1)).split(", ")
#     for i in codes:
...
#
#         #b. Check number of icd-10 codes in dict_code
# print(len(dict_code.keys()))
